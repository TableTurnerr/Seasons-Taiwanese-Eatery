...
const BlogEditFormClient: React.FC<BlogEditClientProps> = ({ initialMetadata }) => {
  const router = useRouter();
  const storage = getStorage();
  
  const [title, setTitle] = useState<string>('');
  const [editorContent, setEditorContent] = useState<JSONContent | null>(null);
  const [initialEditorContentJSONString, setInitialEditorContentJSONString] = useState<string | null>(null); // To track if content changed
  ...
  const [currentVideoStoragePath, setCurrentVideoStoragePath] = useState<string | undefined>(undefined);
  ...
  const [newVideoStoragePath, setNewVideoStoragePath] = useState<string | null>(null);
  ...
  const [currentContentStoragePath, setCurrentContentStoragePath] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    if (!initialMetadata || !initialMetadata.id || !initialMetadata.contentStoragePath || !initialMetadata.restaurantId) {
      if (initialMetadata) { 
          setError("Required information to fetch blog content is missing from metadata.");
      }
      setContentLoading(false);
      return;
    }
    ...
    const fetchContentFromStorage = async () => {
      ...
      try {
        const pathReference = storageRef(storage, initialMetadata.contentStoragePath);
        const downloadURL = await getDownloadURL(pathReference);
        
        const response = await fetch(downloadURL);
        if (!response.ok) {
          throw new Error(`Failed to download content file: ${response.status} ${response.statusText}`);
        }
        ...
        if (contentString) {
          ...
          try {
            ...
            console.error("Error parsing blog content JSON from storage:", parseError);
            setError("Blog content is malformed and could not be displayed.");
            setEditorContent({ type: "doc", content: [] });
            return;
          }

          if (parsedJsonContent && typeof parsedJsonContent === 'object' && 
              parsedJsonContent.type === 'doc' && Array.isArray(parsedJsonContent.content)) {
            setEditorContent(parsedJsonContent);
          } else {
            console.warn("CLIENT: Parsed content from storage is not a valid Tiptap 'doc':", parsedJsonContent);
        ...
        setError(err instanceof Error ? `Failed to load content: ${err.message}` : 'An unknown error occurred while fetching content.');
        setEditorContent({ type: "doc", content: [] }); // Fallback
      } finally {
        setContentLoading(false);
      }
  ...
  const handleCoverImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      ...
      if (!file.type.startsWith("image/")) {
        ...
        setNewCoverImageFile(null); setImagePreview(currentCoverImageUrl || null);
      ...
      try {
        ...
        setNewCoverImageFile(null); setNewCoverImageBase64(null); setImagePreview(currentCoverImageUrl || null);
        if(coverImageInputRef.current) coverImageInputRef.current.value = "";
      }
    } else {
      setNewCoverImageFile(null); setNewCoverImageBase64(null); setImagePreview(currentCoverImageUrl || null);
  ...
  const handleVideoChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setNewVideoDownloadURL(null); setNewVideoStoragePath(null); 
    ...
    if (files && files[0]) {
        ...
        if (!file.type.startsWith("video/")) {
            ...
            setNewVideoFile(null); setVideoPreviewUrl(currentVideoUrl || null);
  ...
  const uploadVideo = async (file: File) => {
    if (!initialMetadata || !initialMetadata.restaurantId || !initialMetadata.id) {
        setVideoError("Cannot upload video: missing essential blog/restaurant ID.");
        return;
    }
    ...
    uploadTask.on('state_changed',
        ...
        async () => {
            try {
                ...
                setVideoUploading(false);
                setSuccessMessage("Video uploaded successfully. Save post to apply changes.");
            } catch (error) {
  ...
  const deleteOldVideo = async (oldVideoStoragePath: string | undefined) => {
    ...
    try {
        ...
        setError((prevError) => prevError ? `${prevError}. Failed to delete previous video from storage.` : "Failed to delete previous video from storage.");
    }
  };
  ...
  const handleUpdate = async (e?: FormEvent<HTMLFormElement>) => {
    ...
    if (!initialMetadata || !initialMetadata.restaurantId || !initialMetadata.id ) {
      setError("Essential blog information (ID, RestaurantID) is missing. Cannot update.");
      return;
    }
    if (!title.trim()) {
    ...
    const isEmptyTiptapContent = !editorContent || !editorContent.content ||
    ...
    let contentJsonStoragePath = initialMetadata.contentStoragePath || 
    ...
    try {
      const blogMetaDocRef = doc(db, `Restaurants/${initialMetadata.restaurantId}/blogs`, initialMetadata.id);
      ...
      if (newVideoDownloadURL && newVideoStoragePath) {
        ...
        if (currentVideoStoragePath && currentVideoStoragePath !== newVideoStoragePath) {
            oldVideoPathToDeleteAfterSave = currentVideoStoragePath;
        }
      } else if (newVideoFile && !newVideoDownloadURL) {
        setError("Video selected but not fully processed. Please try re-selecting or wait.");
        setIsUpdating(false);
        return;
      }
      
      // Handle blog content update (upload to storage if changed or if path was initially missing)
      const currentEditorContentString = editorContent ? JSON.stringify(editorContent) : JSON.stringify({type: "doc", content: []});
      let contentHasChanged = currentEditorContentString !== initialEditorContentJSONString;

      if (contentHasChanged || !initialMetadata.contentStoragePath) {
        const contentFileRef = storageRef(storage, contentJsonStoragePath);
        await uploadString(contentFileRef, currentEditorContentString, 'raw', { contentType: 'application/json'});
        console.log("Blog content updated/created in Firebase Storage at:", contentJsonStoragePath);
      ...
      setNewCoverImageBase64(null); setNewCoverImageFile(null);
      setNewVideoFile(null); setNewVideoDownloadURL(null); setNewVideoStoragePath(null); 
      setVideoUploadProgress(0);
      if(videoInputRef.current) videoInputRef.current.value = ""; 
      if(coverImageInputRef.current) coverImageInputRef.current.value = "";
      ...
      setError(err instanceof Error ? `Failed to update post: ${err.message}` : "An unknown error occurred during update.");
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 font-creto h-screen overflow-auto flex flex-col bg-gray-50 no-scrollbar">
      <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center text-gray-800">Edit Blog Post</h1>
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded fixed right-10 top-20 z-50 mb-4 shadow-lg opacity-80" role="alert">
      ...
      {successMessage && (
        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded fixed right-10 top-20 z-50 mb-4 shadow-lg opacity-80" role="alert">
      ...
      {videoError && (
         <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded fixed right-10 top-32 z-50 mb-4 shadow-lg" role="alert">
            <strong className="font-bold">Video Error: </strong>
            <span className="block sm:inline">{videoError}</span>
            <button onClick={() => setVideoError(null)} className="absolute top-0 bottom-0 right-0 px-4 py-3 text-red-700">
                <span className="text-2xl">&times;</span>
            </button>
        </div>
      )}
      <form ref={formRef} onSubmit={handleUpdate} className="mx-auto w-full max-w-4xl flex flex-col flex-grow">
        ...
        <div className="mb-6">
            ...
            {videoUploading && (
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2.5">
                    ...
                    <p className="text-xs text-center text-gray-600">{Math.round(videoUploadProgress)}% uploaded</p>
                </div>
            )}
        ...
        <div className="mb-6 flex-grow flex flex-col h-full p-4 md:p-6 bg-white shadow-lg rounded-lg border border-gray-200">
          ...
          {contentLoading ? (
            <div className="flex justify-center items-center min-h-[300px] md:min-h-[400px] border border-gray-300 rounded-md">
              <PulsingDotsSpinner color="currentColor" className="text-primary"/>
      ...
      <div className="flex justify-between items-center mt-auto pt-6 pb-4 border-t border-gray-200">
         <button
            type="button"
            onClick={() => router.push(initialMetadata?.id ? `/blogs/${initialMetadata.id}` : '/blogs')} 
        ...
        >
          {isUpdating ? 'Updating...' : (videoUploading ? 'Uploading Video...' : 'Update Post')}
        </button>
      </div>
    </div>
  );
