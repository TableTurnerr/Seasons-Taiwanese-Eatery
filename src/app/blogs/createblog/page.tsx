...
const CreateBlogPage = () => {
	...
	const handleVideoChange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (files && files[0]) {
			...
			if (file.size > MAX_SIZE) {
				setVideoError(
					`Video file size exceeds ${MAX_SIZE / (1024 * 1024)}MB.`,
				);
				if (videoInputRef.current) videoInputRef.current.value = "";
				return;
			}
	...
	const uploadVideo = async (file: File) => {
		...
		if (!restaurantId) {
			setVideoError(
				"Restaurant ID is not configured. Cannot upload video.",
			);
	...
	const handlePublish = async (e: React.FormEvent) => {
		...
		if (!coverImageAltText.trim()) {
			...
			setError(
				"Cover image alt text is required for accessibility and SEO.",
			);
			return;
		}
		if (videoUploading) {
		...
		try {
			...
			if (
				...
				try {
					...
					setError(
						`${publishError} (Blog document creation was rolled back.)`,
					);
					...
					if (videoStoragePath) {
						...
						const videoToDeleteRef = storageRef(
							storage,
							videoStoragePath,
						);
						await deleteStorageObject(videoToDeleteRef)
							...
							.catch((storageDeleteError) => {
								...
								setError(
									(prev) =>
										`${prev} Failed to delete video file from storage.`,
								);
							});
					}
				} catch (rollbackError) {
	...
	const handleCoverImageChange = async (
		...
		if (files && files[0]) {
			...
			if (!file.type.startsWith("image/")) {
				setError(
					"Invalid file type. Please select an image for the cover.",
				);
				if (coverImageInputRef.current)
					coverImageInputRef.current.value = "";
				return;
			...
			if (file.size > MAX_SIZE) {
				setError(
					`Cover image size exceeds ${MAX_SIZE / (1024 * 1024)}MB.`,
				);
				if (coverImageInputRef.current)
					coverImageInputRef.current.value = "";
				return;
			...
			>
					...
					<div>
						...
						<div className="mb-6 flex items-center gap-x-3">
							<input
								...
								onChange={(e) =>
									setPostArchived(e.target.checked)
								}
					...
					<div className="mb-6">
						...
						<div className="mt-4">
							...
							<input
								...
								onChange={(e) =>
									setCoverImageAltText(e.target.value)
								}
				...
				<div className="mb-6">
					<p className="text-sm text-gray-500">
						...
						<li>
							Kindly confirm that the blog has been published by
							checking if the blog is shown on the blogs page
							after getting the successfully uploaded message.
						</li>
						<li>
							Whenever making a new blog archive it, so that it
							will not be shown to users as the blog content page
							will not exist at that point in time.
						</li>
					</p>
				</div>
