...
import Spinner from "@/components/Spinner"; // Corrected Spinner import
...
const BlogsPage = () => {
	...
	const fetchBlogCount = useCallback(async () => {
		if (!restaurantId) {
			console.error(
				"Restaurant ID is not configured for fetching count.",
			);
			return;
		}
		try {
			...
			<div className="flex flex-col justify-center items-center min-h-screen p-4">
				...
				{!restaurantId && (
					<p className="text-sm text-gray-500 mt-2">
						Hint: Restaurant ID might be missing in environment
						variables.
					</p>
				)}
			</div>
	...
	const commonLinkClasses =
		"block bg-white rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 overflow-hidden group";
						...
						{!restaurantId &&
							process.env.NODE_ENV === "development" && (
								<p className="text-sm text-orange-500 mt-1">
									Warning: Restaurant ID
									(NEXT_PUBLIC_FIREBASE_RESTAURANT_ID) is not
									set.
								</p>
							)}
					</div>
										...
										blog.id != "HhrqYszvCsMXXo2gywSO" && (
											...
											>
												...
												>
													...
													<div className="p-6">
														<h2
															className="text-2xl font-semibold text-gray-800 mb-2 group-hover:text-primary transition-colors duration-300 truncate"
															title={blog.title}
														>
															{blog.title}
														</h2>
														...
														<div className="mt-4 flex justify-between">
															<span className="inline-block bg-primary-light text-primary text-sm font-semibold px-3 py-1 rounded-full group-hover:bg-primary group-hover:text-white transition-colors duration-300">
																Read More &rarr;
															</span>
															...
															{user && (
																<div className="flex items-center gap-2">
																	...
																	<Link
																		...
																		legacyBehavior
																	>
																		<span className="text-gray-500 text-xs hover:text-primary transition-colors duration-300">
																			<EditIcon />
																		</span>
																	</Link>
																</div>
											...
											>
												...
												>
													...
													<div className="flex-grow text-center sm:text-left">
														<h2
															className="text-xl md:text-2xl font-semibold text-gray-800 mb-1 group-hover:text-primary transition-colors duration-300"
															title={blog.title}
														>
															{blog.title}
														</h2>
