...
import customImage from "@/components/tiptap-templates/simple/customImage";
...
const BlogDisplayClient: React.FC<BlogDisplayClientProps> = ({
			...
			try {
				...
				if (contentString) {
					...
					if (
						...
						setContentError(
							"Blog content format from storage is invalid.",
						);
				...
				console.error(
					"CLIENT: Error fetching blog content from storage:",
					err,
				);
				setContentError(
	...
	useEffect(() => {
		if (editor && blogContent) {
			if (
				JSON.stringify(editor.getJSON()) !== JSON.stringify(blogContent)
			) {
	...
	return (
		<div className="font-creto overflow-y-auto min-h-screen">
			<article className="bg-white shadow-xl rounded-lg">
					...
					{blogMetadata.coverImage && (
						<div className="relative w-full h-64 md:h-96 mb-6 sm:mb-0">
							<Image
								...
								layout="fill"
								objectFit="contain"
								className="rounded-3xl contain-layout"
								priority
								unoptimized={true}
							/>
						</div>
